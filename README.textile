...
<b>Please note that all instructions assume that you are using Unity 3.0!</b>
...
<i>If you are not using Unity 3.0, you will need to use one of the other installation methods below.</i>
...
__If you are not using Unity 3.0, replace the line @git checkout Unity3@ with either @git checkout master@ (for Unity 2.6.x), or @git checkout iPhone@ (for Unity/iPhone 1.x) as appropriate.__
...
bc.   cd myproject/
  git submodule add -b Unity3 git://github.com/MrJoy/UnityREPL.git
  git submodule init
...
* Unity 2.6.0: Untested, should work.  Use the "master" branch.
* Unity 2.6.1: Works.  Use the "master" branch.
* Unity/iPhone 1.x: Sort of works.  Use the "iPhone" branch. (Functional, but limited UI.)
* Unity 3.0.0: Works.  Use the "Unity3" branch.
...
[Package]http://github.com/downloads/MrJoy/UnityREPL/UnityREPL_0.5_Unity3.unitypackage